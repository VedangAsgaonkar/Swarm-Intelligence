...
Experimented with the PI/8 sectors thing. Does not work well.
*/
...
*/
/*Log: Vedang 02-05-2021 20:03
Created v9. Changed source location. Since source is far, increased number of ants to 300. Will experiment with other params.
Also implemented increased step-size as curr_step_size in regions of high pheromone, to reduce accumulation
*/
/*Log: Vedang 02-05-2021 20:09
In v9, incresed number of ants to 500. Much better results. But fear that it may get stuck at local optima on more complicated paths.
The fundamental problem of non-straight line following of pheromone trail remains.
*/
/*Log: Vedang 02-05-2021 21:03
In v9, increased to 900 ants. Also increased weight to 2.5 from 0.25. Works well, but too disciplined. However variation is eratic. 
weight =1 doesnot work well.
The fundamental problem is slower speed along pheromone containing paths.
*/
/*Log: Vedang 02-05-2021 21:28
Freezing v9.
*/
/*Log: Vedang 02-05-2021 21:38
Implemented 2 obstacles in v10. Manages to find a path. But not perfectly optimal. Primarily because of the inabilty to go around
sharp corners. However should not be too big an issue.
*/
/*Log: Vedang 02-05-2021 22:01
Tweaked the large step size to 50. Reduced decay rate. Also sped up the model. Seems like in most cases, it does converge to the 
optimal path. Thus it is recommended to run the model 5-10 times, before concluding an optimal path. Also an automatic stop can
be made using some control parameters of convergence.
*/
/*Log: Vedang 02-05-2021 22:09
Freezing v10.
*/
/*Log: Vedang 03-05-2021 09:03
v11 implements v10 with several obstacles. Very successful!
*/
	...
	void refreshAll(){
		...
		for(int i = 0 ; i < 1000 ; i++){
			if(arr[i]->isDormant == false){
				...
				double ph_qty = 0 ;
				double max_ph = 0 ;
				double curr_step_size = hyperparams::step_size; 
				...
				if( ref_top || ref_bottom || ref_right || ref_left ){
					...
					if( ph_qty > hyperparams::long_step_thr ){
						curr_step_size = 50 ;
					}
					
					double center = distribution1(generator) ;
					double final_x = x0 + curr_step_size*cos(center+shift);
					double final_y = y0 + curr_step_size*sin(center+shift); 
					
					if( (final_x < 450 && final_x > 300 && final_y < 600 && final_y > 100) ){
					//This is the obstacle
					//make sure obstacle is not smaller than step size, else things can just teleport across it
							
						if(	x0 >= 450 || x0 <= 300 ){
							final_x = x0 - 0.1*curr_step_size*cos(center+shift) ;
						}
						if(	y0 >= 600 || y0 <= 100 ){
							final_y = y0 - 0.1*curr_step_size*sin(center+shift) ;
						}		
					}
					if((final_x>500 && final_x<600 && final_y > 600 && final_y<700)){
					//another obstacle
						if(	x0 >= 600 || x0 <= 500 ){
							final_x = x0 - 0.1*curr_step_size*cos(center+shift) ;
						}
					...
					if((final_x>650 && final_x<750 && final_y > 750 && final_y<850)){
					//another obstacle
						if(	x0 >= 750 || x0 <= 650 ){
							final_x = x0 - 0.1*curr_step_size*cos(center+shift) ;
						}
					...
					if((final_x>100 && final_x<300 && final_y > 300 && final_y<400)){
					//another obstacle
						if(	x0 >= 300 || x0 <= 100 ){
							final_x = x0 - 0.1*curr_step_size*cos(center+shift) ;
						}
					...
					if((final_x>700 && final_x<850 && final_y > 300 && final_y<400)){
					//another obstacle
						if(	x0 >= 850 || x0 <= 700 ){
							final_x = x0 - 0.1*curr_step_size*cos(center+shift) ;
						}
...
void callBack( void* target){
	if(run){
		...
		Fl::repeat_timeout(0.05, callBack , target )	;
	}
}
...
int main(){
	...
	MyRectangle* obst1 = new MyRectangle( 300,100,150,500,FL_WHITE);
	MyRectangle* obst2 = new MyRectangle( 500,600,100,100,FL_WHITE);
	MyRectangle* obst3 = new MyRectangle( 650,750,100,100,FL_WHITE);
	MyRectangle* obst4 = new MyRectangle( 100,300,200,100,FL_WHITE);
	MyRectangle* obst5 = new MyRectangle( 700,300,150,100,FL_WHITE);
	Ant* arr[1000] ;
	for(int i = 0 ; i < 1000 ; i++ ){
		arr[i] = new Ant( 800 , 800 , 5 , 5 , 120 , 190 ,240 , 200 , 250);
		arr[i]->currAngle = distribution2(generator);
	...
	Wrapper* wrapper = new Wrapper(arr);
	 // rect = new MyRectangle( 800 , 800 , 5 , 5 , 120 );
	char carr[5] = {'S','t','o','p','\0'} ;
	MyButton* bt = new MyButton( 100 ,900,30,30,carr);
